...
### 5.3 策略
- 回测发现：原策略的高收益是**前视偏差**（打乱基本面分数后从 +51.92% 崩到 +8.69%，中性口径 +9.88%）
- 无偏口径下：固定止盈 +15.45% / 跌破20日线 +5.32% / **移动止损(高点回撤12%) +33.40%**
- 已上线：`trailing_stop_pct=0.12` 替代固定止盈、`etf_target_weight=0.30`、`max_batch_build=2`

## 6. 回测结论（无偏对照，2025-04-09 ~ 2026-09-11，350 个交易日）

| 离场规则 | 总收益 | 最大回撤 | 夏普 | 交易笔数 | 卖出胜率 |
|---|---|---|---|---|---|
| 固定止盈+9%/止损-8% | +15.45% | -10.71% | 0.85 | 106 | 46% |
| 跌破20日均线 | +5.32% | -15.74% | 0.31 | 389 | 32% |
| **移动止损（高点回撤12%）** | **+33.40%** | **-11.58%** | **1.34** | **61** | 30% |
| 对照：等权持有12只 | +20.95% | -8.35% | 1.16 | 12 | — |

> 交易越频繁收益越低（12笔 > 61笔 > 106笔 > 389笔），费用与择时损耗是主要拖累。
>
> 2026-09-13 重跑：区间推进到 09-11，报告/PPT/素材包里的回测数字已改为从 `data/backtest_result.txt` 动态读取；
> 同日发现腾讯日K在某段时间给出偏离真实收盘价 0.3%~1.7% 的脏数据（沪深300 给 4493.15，新浪/东财均为 4510.16），
> 已加指数收盘价自动校正、清空重建日K缓存，并给全市场选股脚本加了双源价差校验。上表为**校正后**结果
> （校正前移动止损口径只有 +18.48%，被脏数据压低了约 15 个百分点）。

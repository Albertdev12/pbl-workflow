...
周六 10:00 照常出周报（不含 AI）。名义上每天最多 3 次 AI 分析。

> **⚠️ 关于定时投递的真实情况（实测数据，务必了解）**
>
> GitHub 免费额度的 `schedule` 触发是**尽力而为**，实测本仓库的历史记录：
>
> | 计划 | 实际落地 | 结果 |
> |---|---|---|
> | 09-10 16:00/16:25 | 18:16 | 4 个 cron 只投递了 1 个 |
> | 09-09 14:00/14:25 | 20:50 / 21:01 | 延迟约 6.8 小时 |
> | 09-09 16:00/16:25 | 18:25 / 19:36 | 延迟约 2.4 小时 |
>
> 结论：**不要依赖 cron 的准点性**。好在 AI 推荐是 1~3 周的波段建议（不是当日打板信号），
> 晚上拿到"明天的清单"完全够用——次日开盘前照单执行即可。
>
> **每天实际能稳定拿到的**：傍晚~夜间那次 eod 运行产出的**下一交易日计划**。
> 想要当天盘中刷新，用下面任一种：
> - 手机仪表盘「高级面板 → 触发收盘全流程」（30 秒）
> - 外部准时触发器（见本文「二点五」节：cron-job.org / Cloudflare Worker）
>
> **幂等保护**：同一「交易日+时段」只生成一次（时段按北京时间自动判定盘前/盘中/盘后）。
> 所以挂多个冗余 cron 是**免费的安全网**——多投递的运行会自动跳过 AI，不会重复调用 API、
> 不会重复提交、不会互相覆盖仪表盘。需要强制重跑时设环境变量 `AI_FORCE=1`。

### 5.3 数据来源与准确性保障（三层）

1. **权威口径取价**：估值/推荐价一律用东方财富快照的**未复权真实成交价**
   （前复权序列在 ETF、除息日会有千分之一级偏差，已实测确认并规避）。
2. **双源交叉校验**：每只候选的收盘价由「东方财富快照价」与「日K收盘价」逐只比对，
   偏差 > 0.5% 的直接剔除，**不进模型**；模型也拿不到不可信的数字。
3. **防幻觉清洗**：模型返回的代码必须存在于当次候选表（幻觉代码直接丢弃），
   买入区间/止损/目标价全部锚定真实收盘价并按 ATR 约束在合理区间内。

数据源：日K采用**三级降级**——东方财富(`push2his`) → 腾讯 → 新浪；价格与宽度用东方财富
全市场快照。实测腾讯日K被限流（30/30 失败）、东财明文 http 返 502 时，新浪仍稳定可用
且数据准确（601899 收 34.11、300750 09-09 收 336.84 与东财完全一致）。
`clist` 主机首次调用自动探测并缓存，云端走 http 主源、本机明文 http 被网关拦截时自动切 https。
